...
底層使用 Docker 的 `--cpus` 和 `--memory` 參數，映射到 Linux cgroups v2 資源控制。

### 共享資源

所有實例共用：
- **Docker image**：`claude-sandbox`，只 build 一次
...
接受一個 `<name>` 參數（預設 `default`），支援 `SANDBOX_CPUS` 和 `SANDBOX_MEMORY` 環境變數。

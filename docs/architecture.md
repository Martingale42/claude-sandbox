...
│  │                                                             │      │
│  │  sshd (:22) ◄─── port-forward ◄─── host :2222 ◄────────────┘      │
│  │    │                                                               │
│  │    └──► claude user                                                │
│  │          ├─ tmux session "claude"                                  │
│  │          │   └─ claude --dangerously-skip-permissions              │
│  │          ├─ ~/.claude/ (skills, plugins, settings, credentials)    │
...
```bash
# 目前的做法：snapshot
docker cp ~/Code/project claude-sandbox:/home/claude/workspace/
...
```
setup.sh 產生 key pair
    │
    ├── .ssh/sandbox_key       ← 私鑰，留在 host（或複製到 Mac）
    └── .ssh/sandbox_key.pub   ← 公鑰，掛載到 container 的 authorized_keys
```

- key pair 是 **per-sandbox** 的，不會汙染你的個人 SSH key
...
| 層 | 職責 |
|----|------|
| System packages | 提供開發所需的基礎工具鏈（git、ripgrep、python3、build-essential 等） |
...
- container 已存在就先砍再建
- SSH config 已存在就跳過
- skills/plugins 目錄先刪再複製（避免 `docker cp` 巢狀問題，見下方）
...
Rust、uv、Claude Code 都裝在使用者目錄（`~/.cargo/bin`、`~/.local/bin`），不在系統 PATH 裡。我們需要確保 SSH 連入時能找到這些工具。
...
`launch-claude.sh` 用的是非互動式模式（`ssh claude-sandbox "tmux new-session ..."`），這時 `.bashrc` 和 `.profile` 都不會被讀取。
...
```bash
# 跑多個 sandbox，用不同 port
SSH_PORT=2223 CONTAINER_NAME=claude-sandbox-2 ./setup.sh
```
